...
const AdvancedSearch = () => {
  ...
  const currentConfig =
    activeTab === "opportunities"
      ? OPPORTUNITY_FORM_CONFIG
      : PROPOSAL_FORM_CONFIG;
  const currentOpenSections =
    activeTab === "opportunities" ? openOppSections : openPropSections;
  ...
  const toggleAllSections = () => {
    ...
    if (activeTab === "opportunities") setOpenOppSections(newOpen);
    else setOpenPropSections(newOpen);
  };
  ...
  const handleSearch = () => {
    const currentFormData =
      activeTab === "opportunities" ? opportunitiesFormData : proposalsFormData;
  ...
  useEffect(() => {
    const loadRecentSearch = async () => {
      try {
        ...
        const showType = viewPage.content?.Data?.ShowType || 0;

        setIsLoadingRecentSearch(true);
        const recentSearchResult = await getRecentSearchData();
        setActiveTab(
          recentSearchResult.rawData?.ResultType == 2
            ? "proposals"
            : "opportunities"
        );
        ...
        console.error(
          "❌ Error loading recent search data for opportunities:",
          error
        );
      } finally {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <h1 className="text-2xl font-semibold text-blue-900">
              Advanced Search -{" "}
              {activeTab === "opportunities" ? "Opportunities" : "Proposals"}
            </h1>
            <button
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              className="flex items-center space-x-2 px-4 py-2 text-gray-700 hover:text-gray-900 border border-gray-300 rounded-md hover:bg-gray-50"
            ...
            <div className="px-6 py-4">
              {activeTab === "opportunities" ? (
                ...
                <DynamicFormRenderer
                  ...
                  handleSelectChange={(name, value) =>
                    setProposalsFormData((prev) => ({ ...prev, [name]: value }))
                  }
                  handleSearch={handleSearch}
